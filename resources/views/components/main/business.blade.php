...
@php
     ...
     *
     * overlay 의 두 번째 정지점을 % 가 아니라 480px 로 잡은 이유:
     * 호버로 카드가 넓어질 때 % 기준이면 어두운 구간이 같이 늘어나 글자 뒤가 밝아진다.
     * 글자 폭(442px) + 좌측 패딩만 px 로 고정해야 어느 너비에서도 흰 글자가 읽힌다.
     */
    $items = [
        [
            'key' => 'care',
            ...
            'overlay' => 'linear-gradient(100deg, rgba(4,113,139,0.94) 0%, rgba(4,113,139,0.86) 480px, rgba(54,148,171,0.5) 100%)',
        ],
        [
            'key' => 'platform',
            'icon' => 'box',
            ...
            'overlay' => 'linear-gradient(100deg, rgba(53,28,150,0.94) 0%, rgba(53,28,150,0.86) 480px, rgba(91,55,237,0.5) 100%)',
        ],
    ];
@endphp
    ...
    <div class="mx-auto flex max-w-content flex-col items-center gap-8 px-6 py-[120px]">
        ...
        <div class="flex w-full flex-col items-stretch gap-10 lg:flex-row" x-data="{ hovered: null }">
            @foreach ($items as $item)
                {{--
                    호버로 너비가 바뀌면 본문 줄 수가 달라져 높이가 들쑥날쑥해진다.
                    시안(139:654)의 카드 높이 420px 로 고정해 흔들림을 없앤다.
                --}}
                <div class="flex transition-[flex-grow] duration-500 ease-out lg:h-[420px] lg:flex-1"
